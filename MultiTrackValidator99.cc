  ...
  */
  ...
  for (unsigned int ww=0;ww<associators.size();ww++){
	...
	dR_trk[i] = std::sqrt(dR);
      ...
      ////std::cout<<"Debug : "<<debugCounter<<std::endl; ++debugCounter; //0


      Int_t eveNumber = event.id().event();
      Int_t runNumber = event.id().run();
      Int_t lumNumber = event.id().luminosityBlock();
      //std::cout<<"Initialising numbers "<<eveNumber<<" "<<runNumber<<" "<<lumNumber<<std::endl;
      //char bufferstring[512];

      // sprintf(bufferstring,"mkdir ./Clusters/Run_%d_Lumi_%d_Event_%d",runNumber,lumNumber,eveNumber);
      //
      // system(bufferstring);

      auto threadId = std::this_thread::get_id();

      stringstream streamThreadId;
      streamThreadId << threadId;

      // long int msTime; struct timeval timeNow;
      // gettimeofday(&timeNow,NULL);
      //
      // msTime = timeNow.tv_sec * 1000 + timeNow.tv_usec / 1000;

      string bufferstring = "./Clusters/DoubletClusters_";
      //string bufferstringLabels = "./Clusters/doubletClustersLabels_";
      bufferstring += streamThreadId.str();
      //bufferstring +="_";
      // bufferstring += std::to_string(msTime);
      // bufferstring +="_Run_";
      int eachHundredEvents = eveNumber;///300;

      bufferstring += std::to_string(runNumber);
      bufferstring +="_Lumi_";
      bufferstring += std::to_string(lumNumber);
      bufferstring +="_Event_";
      bufferstring += std::to_string(eachHundredEvents);

      bufferstring +=".root";

      //std::cout<<"Initialising filename "<<bufferstring<<std::endl;

      //sprintf(bufferstring,"./Clusters/doubletClusters_Run_%d_Lumi_%d_Event_%d.root",runNumber,lumNumber,eveNumber);

      ////std::cout<<"Debug : "<<debugCounter<<std::endl; ++debugCounter; //1
      // std::ofstream clusters;
      // std::ofstream clusterslabels;

      TFile* clusterFile = new TFile(bufferstring.data(),"UPDATE");

      //std::cout<<"Initialising file "<<clusterFile->GetName()<<std::endl;

      ////std::cout<<"Debug : "<<debugCounter<<std::endl; ++debugCounter; //2

      Float_t padHalfSize = 4.0;
      Int_t goodTrack = 0;
      Int_t nHitsTrack = 0;
      //Int_t nHitsPixel = 0;
      Int_t trackId,pdgId = 5421369, charge = 5421369;

      Double_t px = 0.0 ,py = 0.0 ,pz = 0.0 ,pt = 0.0;

      //std::cout<<"Declared variables & entering in the tracks cycle"<<std::endl;

      ////std::cout<<"Debug : "<<debugCounter<<std::endl; ++debugCounter; //3
      ...
      for(View<Track>::size_type i=0; i<trackCollection.size(); ++i){
        ...
        //std::cout<<" - "<<trackId<<" p : "<<px<<" "<<py<<" "<<pz<<" "<<pt<<" - charge "<<charge<<" - nHits "<<nHitsTrack<<std::endl;

        //std::cout<<px<<nHitsTrack<<py<<pz<<pt<<charge<<goodTrack<<std::endl;//nHitsPixel<<std::endl;
        ...
        if (isSimMatched) {
            ...
            //std::cout<<"Yes - pdgId "<<pdgId<<std::endl;
  ...
  //std::cout<<pdgId<<trackId<<px<<nHitsTrack<<py<<pz<<pt<<charge<<goodTrack<<nHitsPixel<<std::endl;

  int hitcounter = 0;

  std::vector<TH2D*> clusterVector;

  //std::cout<<"Declared hitcounter, clusterVector & entering hit cycle"<<std::endl;

  for ( trackingRecHit_iterator recHit = track->recHitsBegin();recHit != track->recHitsEnd(); ++recHit )
  {
    //std::cout<<"-------------------------------------------------------"<<std::endl;
    //std::cout<<"Hit no. "<<hitcounter;
    ...
    //std::cout<<"Got : detId "<<hit_detId<<" - subDetId "<<IntSubDetID<<" and recasterd hit and got detector "<<std::endl;

    //std::cout<<"Got : detId  - subDetId and recasterd hit and got detector "<<std::endl;
    // const PixelGeomDetUnit * PixelGeom = dynamic_cast<const PixelGeomDetUnit*> (detector);
    // const PixelTopology * topolPixel = &(PixelGeom->specificTopology());

    SiPixelRecHit::ClusterRef const& cluster = hit->cluster();

    //std::cout<<"Cluster ref "<<std::endl;
    //++nHitsPixel;
    //if(cluster->sizeX()>padHalfSize*2 || cluster->sizeY()>padHalfSize*2) continue;

    ++hitcounter;

    float oX = 0.0; //((*recHit)->globalPosition()).x();
    float oY = 0.0; //((*recHit)->globalPosition()).y();
    float oZ = 0.0; //((*recHit)->globalPosition()).z();

    float locX = 0.0; //((*recHit)->localPosition().x());
    float locY = 0.0; //((*recHit)->localPosition().y());

    //Local2DPoint hitLocal(locX,locY);

    //std::cout<<"Got : cluster & hitcounter++ && locPoint - oX "<<oX<<" - oY "<<oY<<" - oZ "<<oZ<<" - locX "<<locX<<" - locY "<<locY<<std::endl;
    ...
    int layer,side,disk,last;

    //std::cout<<"Got : clusterPlot : "<<clusterPlot<<std::endl;
    ...
    if(IntSubDetID == PixelSubdetector::PixelBarrel)
       {
         ...
         clusterPlot +="_";
         clusterPlot += std::to_string(last);
         clusterPlot +="_";

         //std::cout<<"Detector Barrel : "<<pbdetId.layer()<<" "<<pbdetId.ladder()<<" "<<pbdetId.module()<<" "<<std::endl;
    ...
    if(IntSubDetID == PixelSubdetector::PixelEndcap)
       {
        ...
        //std::cout<<"Detector Forward : "<<clusterPlot<<std::endl;
      ...
      //  clusterPlot +="_";
       ...
       clusterPlot +="_";
       clusterPlot += std::to_string(py);//py
       clusterPlot +="_";
       clusterPlot += std::to_string(pz);//pz
       clusterPlot +="_";
       clusterPlot += std::to_string(pt);//pt
       clusterPlot +="_";

       clusterPlot += std::to_string(pdgId);//Pdg
       clusterPlot +="_";
       clusterPlot += std::to_string(charge);//charge
       clusterPlot +="_";
       clusterPlot += std::to_string(nHitsTrack);//nHits

       //clusterPlot += std::to_string(msTime);//msTime
       //clusterPlot +="_";

       //std::cout<<"Final cluster name : "<<clusterPlot<<std::endl;

       Double_t minClusterX = floor(cluster->x())-padHalfSize;
       Double_t maxClusterX = floor(cluster->x())+padHalfSize;
       ...
       //std::cout<<"Cluster with X : "<<minClusterX<<" - "<<maxClusterX<<"Y : "<<minClusterY<<" - "<<maxClusterY<<std::endl;
       //std::cout<<"Initialised cluster TH2 : "<<hitClusters.GetName();

       for (int nx = 0; nx < hitClusters.GetNbinsX(); ++nx) {
         for (int ny = 0; ny < hitClusters.GetNbinsY(); ++ny) {

          //  Double_t xCenter = hitClusters.GetXaxis()->GetBinCenter(nx);
          //  Double_t yCenter = hitClusters.GetYaxis()->GetBinCenter(ny);

          //  if(xCenter>topolPixel->nrows() || yCenter>topolPixel->ncolumns())
          //   hitClusters.SetBinContent(nx,ny,-655.35);
          //  else
            hitClusters.SetBinContent(nx,ny,0.0);

         }
       }

       //std::cout<<"to zeros "<<std::endl;
      ...
      //std::cout<<"Filled cluster TH2 "<<std::endl;

      clusterFile->cd();

      hitClusters.Write();

      //std::cout<<"Written cluster TH2 on file"<<std::endl;
  ...
  //std::cout<<"================================================================="<<std::endl;
      ...
      clusterFile->Close();
      //std::cout<<"End of track collection & Closing file"<<std::endl;
