...
void SiPixelRecHitsValid::analyze(const edm::Event& e, const edm::EventSetup& es)
{
  ...
  Int_t eveNumber = e.id().event();
  // Int_t runNumber = e.id().run();
  // Int_t lumNumber = e.id().luminosityBlock();
  ...
  for (TrackerGeometry::DetContainer::const_iterator it = geom->dets().begin(); it != geom->dets().end(); it++)
    {
      std::string bufferstring = "./Hits/pdgIds/";
      ...
      int side,disk,last,detCounter=0;

      if(subid==1)
      {
        pbdetId = PXBDetId(detId);
        detCounter = pbdetId.layer();
        last = pbdetId.ladder();
      ...
      if(subid==2)
      {
        ...
        if(disk<7) detCounter += (side-1)*3;
        else if(disk>11) detCounter += (side-1)*3 + 2;
        else detCounter += (side-1)*3 + 1;
	...
	{
    ++hitCounter[detCounter];
    auto const & gs = static_cast<BaseTrackerRecHit const &>(*pixeliter).globalState();

    float x = gs.position.x();
    float y = gs.position.y();
    float z = gs.position.z();

    // std::cout<<"Det : "<<detCounter<<" Pix Hit :"<<hitCounter[detCounter]<<" x = "<<x<<" y = "<<y<<" z = "<<z<<std::endl;

    // hitsFile<<detCounter<<"\t"<<last<<"\t"<<hitCounter[detCounter]<<"\t"<<x<<"\t"<<y<<"\t"<<z<<"\t";
    ...
    hitsFile<<last<<"\t\t"<<hitCounter[detCounter]<<"\t\t";

    // std::cout<<"PDG ID : "<<(*closestit).particleType()<<std::endl;
    hitsFile<<(*closestit).particleType()<<"\t\t"<<x<<std::endl<<std::endl;
	      if (subid==1)
		{ //<----------barrel
