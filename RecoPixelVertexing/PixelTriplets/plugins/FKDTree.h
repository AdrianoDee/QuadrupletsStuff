...
public:
    ...
    {
        //std::cout<<"Search in the box : FKDTree "<<std::endl;
        theIndecesToVisit.clear();
    ...
    {
        //std::ofstream treeHits("Txts/treeHits.txt",std::ofstream::app);
    ...
    void make_FKDTreeFromRecHitsInPhi(const RecHitsSortedInPhi  & sortedHits, const TrackingRegion & region)
    {

        //std::cout<<"Make Tree From RecHitsPhi : in!"<<std::endl;
        static_assert( numberOfDimensions == 3, "Only for 3-dim trees!" );
        //static_assert( (typeof(TYPE) == float), "Float." );
        const float maxDelphi = region.ptMin() < 0.3f ? float(M_PI)/4.f : float(M_PI)/8.f;
        const float safePhi = M_PI-maxDelphi;

        std::vector<RecHitsSortedInPhi::HitWithPhi> phiHits = sortedHits.theHits;
        std::vector<FKDPoint<TYPE,numberOfDimensions>> points;


        for (int i = 0; i!=(int)phiHits.size(); i++) {

            Hit const & hit = phiHits[i].hit();
            auto const & gs = hit->globalState();
            auto loc = gs.position-region.origin().basicVector();
            float lr = loc.perp();
            auto phi = gs.position.barePhi();
