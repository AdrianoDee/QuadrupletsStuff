  ...
  */
  ...
  for (unsigned int ww=0;ww<associators.size();ww++){
	...
	dR_trk[i] = std::sqrt(dR);
      ...
      std::string bufferstring = "./Clusters/"; bufferstring += std::to_string(runNumber);
      bufferstring += "_"; bufferstring += std::to_string(lumNumber);
      std::string command = "mkdir "; command += bufferstring;

      system(command.data());

      bufferstring += "/doubletsClusters_"; bufferstring += std::to_string(eveNumber);
      bufferstring += ".root";
      ...
      //
      //
      // std::ofstream doubletsClusters(bufferstring.data(), std::ofstream::out);
      //
      // bufferstring = "./Clusters/"; bufferstring += std::to_string(runNumber);
      // bufferstring + = "/doubletsLabels_"; bufferstring += std::to_string(eveNumber);
      // bufferstring += ".txt";
      //
      // std::ofstream doubletsLabels(bufferstring.data(), std::ofstream::out);
      //
      // bufferstring = "./Clusters/"; bufferstring += std::to_string(runNumber);
      // bufferstring + = "/doubletsInfors_"; bufferstring += std::to_string(eveNumber);
      // bufferstring += ".txt";
      //
      // std::ofstream doubletsInfos(bufferstring.data(), std::ofstream::out);
