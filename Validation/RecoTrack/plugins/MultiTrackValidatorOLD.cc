...
void MultiTrackValidator::analyze(const edm::Event& event, const edm::EventSetup& setup){
  ...
  // std::cout<<" MultiTrackValidator analyze"<<std::endl;
  ...
  */
  ...
  for (unsigned int ww=0;ww<associators.size();ww++){
	...
	dR_trk[i] = std::sqrt(dR);
      ...
      // std:string filename("./Clusters/clusters_");
      // filename +="Run_"; filename += std::to_string(event.id().event());clusterPlot += "_";
      // filename +="Lumi_"; filename += std::to_string(event.id().luminosityBlock());clusterPlot += "_";
      // filename +="Evt_"; filename += std::to_string(event.id().event());clusterPlot += "_";
      // filename += ".root";
      //
      // TFile* clusterFile = TFile::Open(filename.data(),"RECREATE");
      ...
      // TH2D* secondHitCluster;
      // TH2D* thirdHitCluster;
      // TH2D* fourthHitCluster;
      ...
      // doubletsTree->Branch("secondHitCluster",secondHitCluster);
      // doubletsTree->Branch("thirdHitCluster",thirdHitCluster);
      // doubletsTree->Branch("fourthHitCluster",fourthHitCluster);

      //Indeces
      doubletsTree->Branch("nHits",&nHits);
      doubletsTree->Branch("trackId",&trackId);

      // std::ofstream sizesX ("./sizesX.txt", std::ofstream::app);
      // std::ofstream sizesY ("./sizesY.txt", std::ofstream::app);
      ...
      for(View<Track>::size_type i=0; i<trackCollection.size(); ++i){
        ...
        if (!isSimMatched) {
          ...
          for (size_t j = 0; j < track->recHitsSize(); ++j) {
           ...
           //  std::cout<<" Hit "<<j<<" detector : "<<((detector->type()).name())<<" subdet "<<(detector->type()).subDetector()<<std::endl;
           //  std::cout<<"Event Id: "<<event.id().event()<<"Run Id: "<<event.id().run()<<"Lumi Id: "<<event.id().luminosityBlock()<<std::endl;
            if((detector->type()).isTrackerPixel() && (((detector->type()).isBarrel())||((detector->type()).isEndcap()))){
            //if(((detector->type()).subDetector()==GeomDetEnumerators::SubDetector::PixelEndcap) || ((detector->type()).subDetector()==GeomDetEnumerators::SubDetector::PixelBarrel)){
            //if(typeid(recHit) == typeid(SiPixelRecHit)){
             ...
             //  std::cout<<"Recasting to SiPixelRecHit - DONE "<<std::endl;

            //  sizesX<<cluster->sizeX()<<std::endl;
            //  sizesY<<cluster->sizeY()<<std::endl;
             //  std::string runEventLumi = "Run_";
             //  runEventLumi += std::to_string(event.id().event());runEventLumi += "_";
             //  runEventLumi +="Lumi_"; runEventLumi += std::to_string(event.id().luminosityBlock());runEventLumi += "_";
             //  runEventLumi +="Evt_"; runEventLumi += std::to_string(event.id().event());

            //  std::string clusterPlot = "MISSRun_";
            //  clusterPlot += std::to_string(event.id().event());clusterPlot += "_";
            //  clusterPlot +="Lumi_"; clusterPlot += std::to_string(event.id().luminosityBlock());clusterPlot += "_";
            //  clusterPlot +="Evt_"; clusterPlot += std::to_string(event.id().event());

             //  TDirectory *runDir = clusterFile->mkdir(runEventLumi.data());
              //
             //  std::string trackString = "PDG_";
             //  trackString += std::to_string(pdgID);
             //  trackString +="Track_"; trackString += std::to_string(i);

              // clusterPlot +="_Track_"; clusterPlot += std::to_string(i);

             //  TDirectory *trackDir = runDir->mkdir(trackString.data());
              //
                 std::string clusterPlot = "Hit_";
                 clusterPlot += std::to_string(j);
                 clusterPlot +="_pixelCluster";
              // clusterPlot +="_Det_"; clusterPlot += ((detector->type()).name()); clusterPlot += "_";
              // clusterPlot +="_Px_"; clusterPlot += std::to_string(track->momentum().x());
              // clusterPlot +="_Py_"; clusterPlot += std::to_string(track->momentum().y());
              // clusterPlot +="_Pz_"; clusterPlot += std::to_string(track->momentum().z());
              // clusterPlot +="_Pt_"; clusterPlot += std::to_string(track->pt());
              // clusterPlot +="_Q_"; clusterPlot += std::to_string(track->charge());
             ...
             //  std::cout<<"Avg x : "<<cluster->x()<<" - Avg y : "<<cluster->y()<<std::endl;
             ...
             //  std::cout<<"Making cluster pad from "<<minClusterX<<"to"<<maxClusterX<<" and from "<<minClusterY<<"to"<<maxClusterY<<std::endl;
        ...
        if (isSimMatched) {
          ...
          //std::cout<<"Track matched - Inner Det Id:"<<track->innerDetId()<<" Outer :"<<track->outerDetId()<<std::endl;
